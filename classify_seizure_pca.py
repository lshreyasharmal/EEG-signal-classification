...
def accuracy(y_test,result,classifier, balanced_status):
    print "Accuracy with "+ classifier +" on seizure( PCA on "+balanced_status+") : " + str(accuracy_score(y_test,result))
    print
    ...
    plt.title("ROC CURVE")
    plt.savefig("PCA_"+classifier+" "+balanced_status+".jpg")
    plt.show()
    print
x_train = np.load("x_train_transformed_unbalanced.npy")
x_test = np.load("x_test_transformed_unbalanced.npy")
